...
Ellama has separate **per-function providers**, allowing different models for
different tasks. This adapter maps agentroles roles to the closest Ellama
provider:
...
- Ellama's provider assignment is per-function, not per-interaction. You
  switch providers by changing which Ellama command you invoke.
...
Ellama is a popular Emacs package for integrating LLMs into editing workflows.
It supports multiple backends (OpenAI, Anthropic, Ollama, etc.) through the
`llm` library and allows per-function model assignment.
